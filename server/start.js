...
//The code below works because `.use` returns `this` which is
//T `app`. So what we want to return in the `module.exports` is
//T `app`, and we can chain on that declaration because each method
//T invokation returns `app` after mutating based on the middleware
//T functions
module.exports = app
  ...
  // notice the use of `_` as the first parameter above.
  // This is a pattern for parameters that must exist,
  // but you don't use or reference (or need) in the function body
  // that follows.
